...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onUpload, apiKeyError, files, setFiles, isApiKeySelected, onSelectKey }) => {
    ...
    const handleSelectSample = async (name: string, url: string, fileName: string) => {
        ...
        try {
            ...
            if (error instanceof Error && error.message.includes('Failed to fetch')) {
                alert(`Không thể tải tài liệu mẫu. Vui lòng thử tải lên tệp cục bộ thay thế.`);
            }
        } finally {
    ...
    const handleConfirmUpload = async () => {
        try {
            ...
            console.error("Quá trình tải lên thất bại:", error);
        }
    };
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 lg:p-8">
            <div className="w-full max-w-3xl text-center">
                <h1 className="text-4xl sm:text-5xl font-bold mb-2">Trò chuyện với tài liệu của bạn</h1>
                <p className="text-gem-offwhite/70 mb-8">
                    Được hỗ trợ bởi <strong className="font-semibold text-gem-offwhite">FileSearch</strong>. Tải lên hướng dẫn sử dụng hoặc chọn ví dụ để xem RAG hoạt động.
                </p>
                ...
                <div className="w-full max-w-xl mx-auto mb-8">
                     {!isApiKeySelected ? (
                        ...
                        >
                            Chọn khóa API Gemini để bắt đầu
                        </button>
                    ) : (
                ...
                >
                    <div className="flex flex-col items-center justify-center">
                        <UploadCloudIcon />
                        <p className="mt-4 text-lg text-gem-offwhite/80">Kéo và thả tệp PDF, .txt hoặc .md của bạn vào đây.</p>
                ...
                {files.length > 0 && (
                    <div className="w-full max-w-xl mx-auto mb-6 text-left">
                        <h4 className="font-semibold mb-2">Tệp đã chọn ({files.length}):</h4>
                ...
                <div className="w-full max-w-xl mx-auto">
                    {files.length > 0 && (
                        <button 
                            ...
                            title={!isApiKeySelected ? "Vui lòng chọn khóa API trước" : "Bắt đầu phiên trò chuyện với các tệp đã chọn"}
                        >
                            Upload and Chat
                        </button>
                    )}
                ...
                <div className="text-left mb-4">
                    <p className="text-gem-offwhite/80">Thử một ví dụ:</p>
                </div>
