...
interface ChatInterfaceProps {
    ...
    activeRagStoreName: string | null;
}

const ChatInterface: React.FC<ChatInterfaceProps> = ({ documentName, history, isQueryLoading, onSendMessage, onNewChat, exampleQuestions, activeRagStoreName }) => {
    ...
    const handleViewFiles = async () => {
        ...
        if (activeRagStoreName) {
            ...
            try {
                const fetchedFiles = await geminiService.listFiles(activeRagStoreName);
                setFiles(fetchedFiles);
            } catch (error) {
                console.error("Không thể tải danh sách tệp:", error);
            } finally {
        ...
        <div className="flex flex-col h-full relative">
            <header className="absolute top-0 left-0 right-0 p-4 bg-gem-onyx/80 backdrop-blur-sm z-10 flex justify-between items-center border-b border-gem-mist">
                <div className="w-full max-w-4xl mx-auto flex justify-between items-center px-4">
                    <h1 className="text-2xl font-bold text-gem-offwhite truncate" title={`Trò chuyện với ${documentName}`}>Trò chuyện với {documentName}</h1>
                    <div className="flex items-center space-x-2">
                        <button
                            onClick={handleViewFiles}
                            className="flex items-center px-4 py-2 bg-gem-mist/20 hover:bg-gem-mist/40 rounded-full text-gem-offwhite transition-colors flex-shrink-0"
                            title="View uploaded files"
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                            </svg>
                            <span className="ml-2 hidden sm:inline">Tệp</span>
                        </button>
                        <button
                            onClick={onNewChat}
                        ...
                        >
                            <RefreshIcon />
                            <span className="ml-2 hidden sm:inline">Cuộc trò chuyện mới</span>
                        </button>
                    </div>
                </div>
            </header>
                            ...
                            <div className={`max-w-xl lg:max-w-2xl px-5 py-3 rounded-2xl ${
                                ...
                                {message.role === 'model' && message.groundingChunks && message.groundingChunks.length > 0 && (
                                    <div className="mt-4 pt-3 border-t border-gem-mist/50">
                                        <h4 className="text-xs font-semibold text-gem-offwhite/70 mb-2 text-right">Nguồn:</h4>
            ...
            {modalContent !== null && (
                ...
                >
                    <div className="bg-gem-slate p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
                        <h3 id="source-modal-title" className="text-xl font-bold mb-4">Văn bản nguồn</h3>
                        <div 
