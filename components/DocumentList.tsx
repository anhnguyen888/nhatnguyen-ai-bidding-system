    ...
    if (!selectedStore) {
        return (
            <div className="flex flex-col h-full items-center justify-center text-center text-gem-offwhite/60">
                <p className="text-lg">Chọn một Kho RAG</p>
                <p>để xem và quản lý tài liệu.</p>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-full">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold truncate" title={selectedStore.displayName}>Tài liệu</h2>
                <button
            ...
            {isUploadModalOpen && (
                <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" role="dialog" aria-modal="true" aria-labelledby="upload-doc-title">
                    <div className="bg-gem-slate p-6 rounded-lg shadow-xl w-full max-w-lg">
                        <h3 id="upload-doc-title" className="text-xl font-bold mb-4">Tải lên tài liệu</h3>
                        
                        <div className="mb-4">
                            <label htmlFor="file-upload" className="block text-sm font-medium text-gem-offwhite/80 mb-2">Tệp</label>
                            <input
                                id="file-upload"
                                type="file"
                                ref={fileInputRef}
                            ...
                            />
                            {selectedFile && <p className="text-sm mt-2 text-gem-offwhite/70">Đã chọn: {selectedFile.name}</p>}
                        </div>

                        <div className="mb-4">
                            <h4 className="text-sm font-medium text-gem-offwhite/80 mb-2">Siêu dữ liệu tùy chỉnh (tùy chọn)</h4>
                            <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                                {metadata.map((item, index) => (
                                    <div key={index} className="flex items-center space-x-2">
                                        <input type="text" placeholder="Khóa" value={item.key} onChange={(e) => handleMetadataChange(index, 'key', e.target.value)} className="w-1/2 bg-gem-mist border border-gem-mist/50 rounded-md py-1 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-gem-blue" />
                                        <input type="text" placeholder="Giá trị" value={item.value} onChange={(e) => handleMetadataChange(index, 'value', e.target.value)} className="w-1/2 bg-gem-mist border border-gem-mist/50 rounded-md py-1 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-gem-blue" />
                            ...
                            <button onClick={addMetadataRow} className="mt-2 flex items-center text-sm text-gem-blue hover:text-blue-400" title="Add another metadata field">
                                <PlusIcon /> <span className="ml-1">Thêm siêu dữ liệu</span>
                            </button>
                        </div>
            ...
            {processingFile && (
                <div className="mb-4 p-3 bg-gem-mist rounded-md flex items-center">
                    <Spinner />
                    <span className="ml-3">Đang xử lý: {processingFile}...</span>
                </div>
            )}
            {isLoading && !documents.length ? (
                ...
                <div className="flex-grow flex items-center justify-center text-center text-gem-offwhite/60">
                    <p>Không tìm thấy tài liệu nào. <br /> Nhấp vào biểu tượng tải lên để thêm.</p>
                </div>
            ) : (
                        ...
                        <li key={doc.name} className="p-3 bg-gem-mist rounded-md group">
                             ...
                             {doc.customMetadata && doc.customMetadata.length > 0 && (
                                <div className="mt-2 pt-2 border-t border-gem-mist/50 text-xs">
                                    <h4 className="font-semibold text-gem-offwhite/70 mb-1">Siêu dữ liệu:</h4>
